...
namespace DVLD
{
    public partial class frmListDrivers : Form
    {
        ...
        private void dgvDrivers_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                var hit = dgvDrivers.HitTest(e.X, e.Y);
                if (hit.RowIndex >= 0)
                {
                    // تحديد الصف
                    dgvDrivers.ClearSelection();
                    dgvDrivers.Rows[hit.RowIndex].Selected = true;

                    // إظهار الكونتكست مينيو في المكان المطلوب
                    contextMenuStrip2.Show(dgvDrivers, e.Location);
                }
            }
        }
        ...
        private void dgvDrivers_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                dgvDrivers.ClearSelection(); // إلغاء التحديد السابق
                dgvDrivers.Rows[e.RowIndex].Selected = true; // تحديد الصف اللي ضغطت عليه
                dgvDrivers.CurrentCell = dgvDrivers.Rows[e.RowIndex].Cells[e.ColumnIndex]; // تعيين الخلية الحالية
            }
        }
    }
}
